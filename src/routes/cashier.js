...
router.post('/submitRequest',async(req,res)=>{
    let SQL =  `INSERT INTO requests (IdUsuario,orderRequest,quantity, note, delivery, formPayment, profit, spent, status,deliveryType,createdAt,updatedAt) VALUES ('${req.body.idUser}','${1}','${Number(req.body.quantity)}','${req.body.observacao}', '${1.99}', '${req.body.formaPagamento}', '${Number(req.body.profit)}','${Number(req.body.spent)}','${'Pendente'}','${req.body.dadosEntrega}',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP); `
